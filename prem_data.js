...
var PREM_DATA = {
  ...
  currentUnit: { n:'M3', topic:'Milestone Test 3 - All Units (U21-U30)', status:'Test ready &middot; 40Q, ASMOPSS-style weighted marks (1.25/2.5/5, 100 total, 80 pass bar), matching Milestone Tests 1 and 2&#39;s exact format. Covers cumulative U21-U29 content (U30 was review-only, no new content, so its material is the same U21-U29 content already covered). Dedicated coverage for two carried-forward items from U30&#39;s real result: the force-for-distance bare-recall trade-off, now a confirmed 2-instance recurring gap (dedicated question + applied confirmers), and the new anther-vs-stigma pollen producer/receiver confusion (a bare-recall question plus an applied bee-pollination question). U30&#39;s own two priority closures (3-force multi-step grouping, magnet-pole diagram care) each get one confirming rep. Benchmarked against Milestone Test 2&#39;s 95/100. Built under the ANSWER-BALANCE RULE - position distribution verified exact 10/10/10/10 across all 40 questions (an initial position-rebalancing pass fixed a severe skew via a new bulk-rebalancing script), zero length-margin violations after two rounds of fixes (15 violations found and fixed on the first pass, 3 more surfaced by a subsequent overlap-driven reword and fixed). A Jaccard overlap check against all ten U21-U30 lesson files and all ten U21-U30 test files caught 15 near-duplicates on the first pass (several deliberate confirming-rep questions echoing their own source too closely) - all reworded with fresh scenarios/objects/numbers while preserving the exact skill and answer; a second overlap pass caught 4 more (formulaic template phrasing shared with lesson practice questions) - reworded again down to zero flags. Critically, the reword-then-reverify pass caught SIX stale cross-references left behind by the rewordings (options/why text still naming old objects or old numbers that no longer matched the updated question stem) - all six found and fixed via a full manual line-by-line fact re-derivation, per the standing project rule that automated checks only verify structure, never semantic correctness after a reword. Scoring engine verified via a Node harness (perfect run 40/40, 100/100 marks, correct 16/16/8 section split; all-wrong run 0/40, 0/100 marks). Full 10-year-old role-play verification pass completed end-to-end - instructions clear, no hint-leaking options, all facts accurate, nav confirmed.' },

  lastUnit: {
    n:30, score:18, outOf:20, pct:90, delta:0, prevScore:90, passed:true,
    patternLabel: 'Unit 30 result - Gap-Closure Checkpoint (18/20, 90% - PASS, matches Unit 29&#39;s result, closes the entire U21-U30 cycle)',
    patternInsight: '<strong>18/20 questions (90%)</strong> - PASS, comfortably above the 16/20 bar, matching Unit 29&#39;s result. Section A (basic recall) = <strong>3/4 (75%)</strong>. Section B (applying, &#9733;) = <strong>5/6 (83%)</strong>. Section C (think like a scientist, &#9733;&#9733;) = <strong>10/10 (100%) PERFECT</strong>.',
    analysisLabel: 'Competition C2&#39;s Q24 (3-force grouping) CONFIRMED CLOSED + magnet-pole diagram care CONFIRMED CLOSED &middot; force-for-distance now a confirmed 2-instance recurring gap, new anther-vs-stigma gap found',
    analysisInsight: 'Headline finding: Section C went a PERFECT 10/10, directly validating BOTH of the checkpoint&#39;s priority targets. The two brand-new 3-force grouping questions (Q11, Q12) were both answered correctly, CONFIRMING Competition C2&#39;s Q24 gap (force combination) is now fully closed - the multi-step group-by-direction-first sequence that broke down in U29&#39;s Q13 now holds under fresh framings. The magnet-pole question (Q13) was also correct, CONFIRMING Competition C2&#39;s Q19b/c diagram-reading care habit is closed - the first re-test of that specific loose end since the original C2 competition sitting. Two misses, both isolated from Section C&#39;s clean sweep: Q4 (Section A bare recall) - the identical force-for-distance trade-off miss as U29&#39;s Q2 ("a heavier object lifted" chosen instead of "a longer distance travelled"), now a CONFIRMED 2-instance recurring gap specifically in bare, abstract restatement (applied versions in both U29 and U30 were answered correctly). Q7 (Section B applied) - a NEW, narrower gap: anther-vs-stigma (producer vs receiver of pollen), distinct from the previously-tracked anther-vs-ovary (male/female) confusion, which remains solid (Q14, the anther-vs-ovary confirming rep, was correct). Both gaps are directly targeted by Milestone Test 3&#39;s dedicated coverage.',
    qSummary: '<strong>18 / 20 (90%) - PASS, matching Unit 29&#39;s result, closing out the entire U21-U30 gap-closure cycle.</strong> Headline: Section C PERFECT 10/10 confirms BOTH priority targets closed - Competition C2&#39;s Q24 (3-force grouping) and Q19b/c (magnet-pole diagram care). Force-for-distance bare recall is now a CONFIRMED 2-instance recurring gap (Q4, mirroring U29&#39;s Q2). A NEW anther-vs-stigma gap surfaced (Q7), distinct from the already-solid anther-vs-ovary distinction. Both gaps get dedicated coverage in Milestone Test 3.',
    qBreakdown: []
  },
