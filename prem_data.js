...
var PREM_DATA = {
  ...
  currentUnit: { n:29, topic:'Physics: Energy, Forces &amp; Simple Machines', status:'Lessons + test ready &middot; ninth unit of the U21-U30 gap-closure cycle, extending Units 18-19&#39;s energy content and lightly introducing Competition C2&#39;s Q29 (mechanical energy calculation) plus a plain-language, non-vector introduction to Q24 (force combination). Opens with a &quot;Quick Fix from Unit 28&quot; screen (2 Quick Checks) re-drilling U28&#39;s two real-result gaps - the derive-then-flip 2-step hemisphere check, and the distance-plays-zero-role absolutism - a cross-subject Quick Fix following the U23 precedent (a Physics unit re-teaching a prior gap from a different subject strand). Lesson 1 `Energy_Lesson_SimpleMachinesAndWork.html` (green, 8 screens: pocket map, Quick Fix from Unit 28, Quick Recall from Units 10 &amp; 18, why, &quot;What Does Work Really Mean?&quot; [SVG: pushing a box that moves vs pushing a wall that doesn&#39;t, Work = Force &times; Distance moved, zero work if nothing moves, foreshadowing the hovering-helicopter idea], &quot;Simple Machines: Trading Force for Distance&quot; [SVG of a long, gentle ramp with a heavy ball and a dashed height-reference line], decision tree [lever/ramp/pulley selection], 5Q practice). Lesson 2 `Energy_Lesson_MechanicalEnergyAndCombiningForces.html` (blue, 7 screens: pocket map, Quick Recall from Lesson 1, why, &quot;Calculating Stored Energy &amp; Lifting Force&quot; [formula GPE = mass &times; 10 &times; height, Force = mass &times; 10, worked example: a 2 kg toy drone hovering 5 m, explicitly framing &quot;10&quot; as a taught simplification of gravity&#39;s real ~9.8 pulling strength, bridging U18 and U21], &quot;Combining Forces: Add or Cancel?&quot; [SVG extending U10&#39;s tug-of-war analogy: same-direction forces add fully, opposite-direction forces subtract, equal-and-opposite forces cancel completely], decision tree, 5Q practice). Test `Tests\\Unit_29_Test_EnergyForcesSimpleMachines.html` (20Q, 4A/6B/10C, PASS_BAR 16) covers work/machine basics, GPE and lifting-force calculations, same-vs-opposite-direction force combination, an embedded Earth &amp; Space re-test of U28&#39;s derive-then-flip gap (Q10) and its distance-absolutism gap (Q18), plus Section C critique/synthesis questions (the &quot;10&quot; simplification vs exact truth, a crane-holding-still zero-work question, a 3-force combination problem, a lever/see-saw application, a pulley application, equal-opposite tow-truck cancellation, an 8kg&times;2m-vs-2kg&times;8m trick question testing genuine formula application over surface pattern-matching, and a final two-student helicopter-hovering force-vs-work synthesis question). Force-combination content deliberately excludes the real competition&#39;s vector-triangle geometry (a 3-4-5 right-triangle minimum-resultant problem), per the Executive Summary&#39;s own prior recommendation that this is beyond G4 level - only the plain-language same-direction-adds/opposite-direction-subtracts rule is taught. Position distribution exact 5/5/5/5 after fixing two drafting errors (Q14, Q15 initially at the wrong index). An initial length-margin pass found 11 questions over a 25% margin (up to 118.8%) - rebalanced down to zero violations. A Jaccard overlap check against both new lessons caught 3 near-duplicates (all against Lesson 2&#39;s own practice questions, echoing the GPE-calculation and same-direction-force wording) - all three reworded with fresh scenarios (a cliff-edge rock, a shopper&#39;s rice sack, a school sports-day box-carry) while preserving the exact formula/skill tested; a stricter cross-unit check against Units 10, 18 &amp; 21&#39;s own lesson files found zero overlaps. Two SVG diagrams were rasterized and visually verified - a confusingly-worded ramp diagram (describing two ramp scenarios but showing only one) and a text-overlap bug in the combining-forces diagram were both found and fixed. Scoring engine verified via a Node harness (perfect run 20/20, all-wrong run 0/20, correct 4/6/10 section split). Full 10-year-old role-play verification pass completed end-to-end across both lessons and the test - all content confirmed factually accurate and age-appropriate, with &quot;Go to Test &rarr;&quot; nav confirmed on Lesson 2&#39;s final screen.' },

  lastUnit: {
    n:28, score:16, outOf:20, pct:80, delta:-20, prevScore:100, passed:true,
    patternLabel: 'Unit 28 result - Earth &amp; Space: Earth&#39;s Tilt &amp; the Seasons (16/20, 80% - PASS, right at the bar)',
    patternInsight: '<strong>16/20 questions (80%)</strong> - PASS, right at the 16/20 bar, a sharp drop from Unit 27&#39;s perfect score. Section A (basic recall) = <strong>4/4 (100%)</strong>. Section B (applying, &#9733;) = <strong>3/6 (50%)</strong>. Section C (think like a scientist, &#9733;&#9733;) = <strong>9/10 (90%)</strong>.',
    analysisLabel: 'Competition C2&#39;s Q25 gap (hemisphere-reversed seasons) PARTIALLY closed &middot; a precise derive-then-flip gap and a distance-absolutism gap both isolated',
    analysisInsight: 'Headline finding: two distinct, narrowly-isolated gaps, not a broad misunderstanding. The hemisphere-reversal rule holds firmly whenever the season word is given directly (Q12) or reasoned about abstractly (Q7, Q17, Q19) - but breaks down specifically when Prem must first DERIVE one hemisphere&#39;s season himself from a physical description or scene (a family planting spring flowers, someone raking fallen leaves) before flipping it for the other hemisphere: both Q6 and Q10 were missed, with the SAME season given for both hemispheres instead of the correct opposite. A second, related gap: the &quot;distance doesn&#39;t cause seasons&quot; myth-bust is only partially absolute - Q9 and Q18 were both missed, with Prem still giving distance partial credit when asked point-blank whether it plays ANY role at all, even though the underlying relative facts (Q11, Q13, Q16) are all solid. Both gaps are precise and narrow rather than a fresh conceptual misunderstanding - directly targeted by Unit 29&#39;s opening Quick Fix screen (a 2-step derive-then-flip check, plus a zero-percent-distance drill).',
    qSummary: '<strong>16 / 20 (80%) - PASS, right at the bar.</strong> Headline: Competition C2&#39;s Q25 gap (hemisphere-reversed seasons) is PARTIALLY closed - the reversal rule holds when the season word is given directly or reasoned about abstractly, but breaks down when Prem must derive one hemisphere&#39;s season himself before flipping it (Q6, Q10). The distance-myth-bust is also only partially absolute (Q9, Q18). Section A held a perfect 4/4; Section B dropped to 3/6, the weakest Section B result since Unit 22&#39;s fail.',
    qBreakdown: []
  },
